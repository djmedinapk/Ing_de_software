...
public class DAOtienda
{
    ...
    public DataTable agregar_carrito(int id_usuario, int id_producto, int cantidad )
    {
        DataTable carrito = new DataTable();
        NpgsqlConnection conectar = new NpgsqlConnection(ConfigurationManager.ConnectionStrings["Postgres"].ConnectionString);
        try
    ...
    public DataTable consultar_cantidad(int id)
    {
        DataTable cantidad = new DataTable();
        NpgsqlConnection conectar = new NpgsqlConnection(ConfigurationManager.ConnectionStrings["Postgres"].ConnectionString);
        try
        {
            NpgsqlDataAdapter dataAdapter = new NpgsqlDataAdapter("f_obtener_cantidad", conectar);
            dataAdapter.SelectCommand.CommandType = CommandType.StoredProcedure;
            dataAdapter.SelectCommand.Parameters.Add("_idproducto", NpgsqlDbType.Integer).Value = id;
            conectar.Open();
            dataAdapter.Fill(cantidad);
        }
        catch (Exception Ex)
        {
